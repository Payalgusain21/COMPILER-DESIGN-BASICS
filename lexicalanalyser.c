//PROGRAM TO CREATE A SIMPLE LEXICAL ANALYSER THAT IDENTIFY KEYWORDS IDENTIFIER AND OPERATORS
#include <stdio.h>
#include <ctype.h>
#include <string.h>
...
int main() {
    FILE *fp;
    char ch, buffer[100];                  //buffer to store token
    ...
    while ((ch = fgetc(fp)) != EOF) {          //read characters of file  
        if (isalnum(ch) || ch == '_') {        //build token if character is alphanumeric or underscore
            buffer[i++] = ch;
        } else {
